...
# prompted for via 'question'/'options' (skipped entirely if --dry-run is set), using
# any value already supplied as the default. After confirmation the container is
...
_container_create() {
    ...
    if [[ -z "$name" && "$prompt_mode" == "true" && "$dry_run" != "true" ]]; then
