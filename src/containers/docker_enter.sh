...
# Description: Opens an interactive terminal session inside a Docker container.
...
#
# Examples:
#   docker_enter --name myapp
#   docker_enter --name webserver --user www-data --shell /bin/sh
function docker_enter {
    local name=""
    local user="root"
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -s|--shell)
                ...
                ;;
            *)
                error "docker_enter: unknown option: $1"
                return 1
                ;;
        esac
    done
    ...
    if [[ -z "$name" ]]; then
        error "docker_enter: --name is required"
        return 1
    fi

    if ! command -v docker &>/dev/null; then
        error "docker_enter: Docker is not installed. Run docker_create first or install Docker manually."
        return 1
    fi

    if ! docker inspect "$name" &>/dev/null; then
        error "docker_enter: container '$name' does not exist"
        return 2
    fi

    local running
    running=$(docker inspect --format='{{.State.Running}}' "$name" 2>/dev/null)
    debug "docker_enter: container '$name' running='$running'"
    ...
    docker exec -it -u "$user" "$name" "$shell"
}
