...
container() {
    ...
    case "$_command" in
        ...
        *)
            error "container: unknown command: '$_command'. Valid commands: create, config, start, exec, enter, push, pull, delete, network"
            return 1
            ;;
    esac
}
...
_container_config() {
    ...
    if [ "$backend" = "pct" ]; then
        pct_config --vmid "$name" "$@"
    else
        lxc_config --name "$name" "$@"
    fi
}
...
_container_create() {
    ...
    if [ "$backend" = "pct" ]; then
        ...
        pct_create --vmid "$vmid" --hostname "$name" "$@"
    else
        lxc_create --name "$name" "$@"
    fi
}
...
_container_push() {
    ...
    if [ "$backend" = "pct" ]; then
        pct_push "$source_path" "$dest_path" --vmid "$name" "$@"
    else
        lxc_push "$source_path" "$dest_path" --name "$name" "$@"
    fi
}
...
_container_pull() {
    ...
    if [ "$backend" = "pct" ]; then
        pct_pull "$source_path" "$dest_path" --vmid "$name" "$@"
    else
        lxc_pull "$source_path" "$dest_path" --name "$name" "$@"
    fi
}
...
_container_network() {
    ...
    if [ "$backend" = "pct" ]; then
        pct_network --vmid "$name" "$@"
    else
        lxc_network --name "$name" "$@"
    fi
}
...
_container_delete() {
    ...
    if [ "$backend" = "pct" ]; then
        pct_delete --vmid "$name" "$@"
    else
        lxc_delete --name "$name" "$@"
    fi
}
