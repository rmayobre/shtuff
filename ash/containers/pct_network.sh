...
pct_network() {
    ...
    if [ "$dry_run" = "true" ]; then
        ...
        [ -n "$ip" ] && dry_net_string="${dry_net_string},ip=${ip}"
        if [ -n "$ip" ] && [ "$ip" != "dhcp" ] && [ -n "$gateway" ]; then
            dry_net_string="${dry_net_string},gw=${gateway}"
        fi
        printf '[DRY RUN] pct set %s --net%s "%s"\n' "$vmid" "$index" "$dry_net_string"
        [ -n "$dns" ] && printf '[DRY RUN] pct set %s --nameserver "%s"\n' "$vmid" "$dns"
        return 0
    fi
    ...
    mkfifo "$_fifo"
    log_output < "$_fifo" &
    pct set "$vmid" "--net${index}" "$net_string" > "$_fifo" 2>&1 &
    local _pid=$!
    rm -f "$_fifo"
    ...
    if [ -n "$dns" ]; then
        ...
        mkfifo "$_fifo2"
        log_output < "$_fifo2" &
        pct set "$vmid" --nameserver "$dns" > "$_fifo2" 2>&1 &
        local _pid2=$!
        rm -f "$_fifo2"
