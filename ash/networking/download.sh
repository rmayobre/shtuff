...
#              By default the file is saved in the current working directory;
#              use --dir to override the destination.
#
# Arguments:
#   --url URL      (string, required): The URL of the file to download.
#   --dir DIR      (string, optional, default: current directory): Directory
...
download() {
    ...
    # Default destination directory to current working directory.
    if [ -z "$dir" ]; then
        dir="$(pwd)"
    fi
