...
lxc_delete() {
    ...
    if [ "$dry_run" = "true" ]; then
        local dry_destroy_args
        dry_destroy_args="-n \"$name\""
        if [ "$force" -eq 1 ]; then
            dry_destroy_args="${dry_destroy_args} -f"
        fi
        printf '[DRY RUN] lxc-destroy %s\n' "$dry_destroy_args"
        return 0
    fi
    ...
    if ! command -v lxc-destroy >/dev/null 2>&1; then
