...
#   -n NAME, -d DESC, -c SPEC, -b TIME, -a TIME, -i TIME, -r TIME, -p, -u NAME,
#   -w TARGET, -o DIR, -f, -h: Short forms of the above flags.
#
# Globals:
...
timer() {
    ...
    if [ -n "${description}" ]; then
        ...
        content="${content}Description=Timer for ${name}\n"
    fi
    ...
    if [ -n "${on_calendar}" ]; then
        content="${content}OnCalendar=${on_calendar}\n"
    fi

    if [ -n "${on_boot_sec}" ]; then
        content="${content}OnBootSec=${on_boot_sec}\n"
    fi

    if [ -n "${on_unit_active_sec}" ]; then
        content="${content}OnUnitActiveSec=${on_unit_active_sec}\n"
    fi

    if [ -n "${on_unit_inactive_sec}" ]; then
        content="${content}OnUnitInactiveSec=${on_unit_inactive_sec}\n"
    fi

    # Additional options
    if [ -n "${randomized_delay}" ]; then
        content="${content}RandomizedDelaySec=${randomized_delay}\n"
    fi

    if [ "${persistent}" = "true" ]; then
        content="${content}Persistent=true\n"
    fi
    ...
    content="${content}WantedBy=${wanted_by}\n"
